...
function filterRestUser(except) {
    ...
    for (var i = 0; i < allUsers.length; i++) {
        ...
        // console.log(user)reward-pepole
...
function completedOnceRolling() {
    if (!isRewardCompleted(current)) {
        ...
        if (task.lastRandUsers.length > 0) {
            ...
            // console.log(task.restUsers+"111111")
            var count = 0;
            ...
            logger.info('Lottery taskId ' + current.taskId + ' 余下:' +count+'人'+ tasks[current.taskId].restUsers.toString());
            return true;
        }
    }
    return false;
